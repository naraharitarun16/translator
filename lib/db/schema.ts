...
export const messages = pgTable('message', { id: text('id').primaryKey(), userId: text('userId').notNull(), senderName: text('senderName').notNull(), senderHandle: text('senderHandle').notNull(), sourceLanguage: text('sourceLanguage').notNull(), body: text('body').notNull(), createdAt: timestamp('createdAt').notNull().defaultNow() })
